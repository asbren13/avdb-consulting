...
export default function Navigation({ showLogo = true, lightNav = false }) {
  ...
  return (
    <div className={classNames('flex', {
      ...
      <div
        className={classNames('mobile-nav bg-red w-full flex flex-col justify-end pt-10 pr-16 pl-14', {
          closed: !isOpen,
        })}
      >
	    <div className="close-button-container flex justify-between">
	      <img className="nav-logo" src={appLogoCream} alt="AVDB Consulting" />
          <button type="button" className="close-button" tabIndex="0" onClick={() => setIsOpen(false)}>
            <div className="close-left" />
            <div className="close-right" />
          </button>
        </div>
        <div className="mobile-menu h-full overflow-auto text-secondary">
	  ...
	  {showLogo && <img className="nav-logo mt-8 ml-14" src={!lightNav ? appLogoSand : appLogoCream} alt="AVDB Consulting" />}
      <div className="button-container flex items-center mt-10 mr-6 p-0">
      	<a className="linkedin-icon mr-2 text-3xl" href="https://www.linkedin.com/in/alisonvdb/" target="_blank" rel="noreferrer noopener">
